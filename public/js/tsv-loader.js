...
(async function () {
  ...
  function buildTercet(lines) {
    ...
    for (var i = 1; i < lines.length; i++) {       // skip header row
      ...
      // Prosody / chondo marker: col-0 wrapped in `*( … )*`. Renders
      // as a small italic caption row between section heading and
      // verse — distinct from both heading and verse text.
      if (bn.length > 4 && bn.indexOf('*(') === 0 && bn.lastIndexOf(')*') === bn.length - 2) {
        var markerBn = bn.substring(2, bn.length - 2).trim();
        var markerEn = hasEn ? col2 : col1;
        // Strip wrapping `*( )*`, `( )`, or `[ ]` from the English so it
        // reads as a clean italic caption matching the Bangla.
        if (markerEn.indexOf('*(') === 0 && markerEn.lastIndexOf(')*') === markerEn.length - 2) {
          markerEn = markerEn.substring(2, markerEn.length - 2).trim();
        } else if (markerEn.length > 2 && markerEn.charAt(0) === '(' && markerEn.charAt(markerEn.length - 1) === ')') {
          markerEn = markerEn.substring(1, markerEn.length - 1).trim();
        }
        html += '<tr class="tt-marker">';
        html += '<td class="bn">'     + markerBn + '</td>';
        html += '<td class="ln-col"></td>';
        if (hasEn) {
          html += '<td class="it"></td>';
          html += '<td class="en">'   + markerEn + '</td>';
        } else {
          html += '<td class="it">'   + markerEn + '</td>';
        }
        html += '</tr>';
        nextIsStart = false;
        continue;
      ...
      // Section heading: col-0 starts with "## " — render across the
      // same column structure as data rows so the Bangla and the
      // translation both sit in their own columns.
      if (bn.indexOf('## ') === 0) {
        var headingBn = bn.substring(3).trim();
        // English/translation lives in the last content column:
        //   3-col TSV (hasEn=false) → col2 (.it slot)
        //   4-col TSV (hasEn=true)  → col3 (.en slot)
        var headingEn = hasEn ? col2 : col1;
        html += '<tr class="tt-section">';
        html += '<td class="bn">'     + headingBn + '</td>';
        html += '<td class="ln-col"></td>';
        if (hasEn) {
          html += '<td class="it"></td>';
          html += '<td class="en">'   + headingEn + '</td>';
        } else {
          html += '<td class="it">'   + headingEn + '</td>';
        }
        html += '</tr>';
        nextIsStart = false;
        continue;
